...
export async function resolveOrCreateUserByEmail(email, role) {
    const database = db();
    ...
    const newDoc = await usersRef.add({ email, name: email.split('@')[0], role: role || 'collaborator', createdAt: FieldValue.serverTimestamp(), projects: [] });
    return newDoc.id;
}
...
export async function sendInvite(projectId, email, role) {
    const database = db();
    if (!database) throw new Error('DB not available');
    const invite = { projectId, email, role: role || 'collaborator', invitedBy: (window.appState && window.appState.currentUser && window.appState.currentUser.uid) || null, status: 'pending', createdAt: FieldValue.serverTimestamp() };
    ...
    try {
        ...
        const actionCodeSettings = { url: (window.location.origin || window.location.protocol + '//' + window.location.host) + '/?invited=true', handleCodeInApp: true };
        if (fb && fb.auth) {
            await fb.auth().sendSignInLinkToEmail(email, actionCodeSettings);
            localStorage.setItem('lastInviteEmail', email);
        }
    } catch (e) { console.error('Failed to send sign-in link:', e); }
    return docRef.id;
}
...
export async function removeMemberFromProject(projectId, uid) {
    ...
    try { await database.collection('users').doc(uid).update({ projects: FieldValue.arrayRemove ? FieldValue.arrayRemove(projectId) : [] }); } catch(e){}
}
...
if (typeof window !== 'undefined') {
    ...
    Object.assign(window._modules.team, { resolveOrCreateUserByEmail, sendInvite, removeMemberFromProject, updateMemberRole });
}

export default { resolveOrCreateUserByEmail, sendInvite, removeMemberFromProject, updateMemberRole };
