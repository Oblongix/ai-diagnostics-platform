...
(async ()=>{
  ...
  page.on('response', async res => {
    ...
    } catch (e) { console.warn('response log failed', e); }
  });

  page.on('console', async msg => {
    try { const args = await Promise.all(msg.args().map(a=>a.jsonValue().catch(()=>a.toString()))); console.log('PAGE LOG', msg.type(), args); } catch(e){}
  });
