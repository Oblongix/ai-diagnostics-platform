...
(async ()=>{
  ...
  await page.evaluate(() => {
    try {
      ...
      if (!window.firebaseServices.auth.currentUser) {
        window.firebaseServices.auth.currentUser = { uid: 'vincentp', email: 'vincentapowell@msn.com' };
      }
    } catch (e) {
  ...
  await page.evaluate(() => {
    try {
      ...
      if (!window.firebaseServices.db) {
        const store = (window.firebaseServices._store || (window.firebaseServices.db && window.firebaseServices.db._store) || (window.__IN_MEMORY_STORE__ || { projects: {} }));
        ...
        const makeCollection = (name) => ({
          ...
          doc(id) {
            return {
              async get() { const d = (store[name] || {})[id]; return { exists: !!d, id, data: () => d }; },
              async update(obj) { store[name] = store[name] || {}; store[name][id] = Object.assign(store[name][id] || {}, obj); }
            };
          },
          async add(obj) { const id = Math.random().toString(36).slice(2,10); store[name] = store[name] || {}; store[name][id] = obj; return { id }; },
          where() { return { async get() { return { docs: [] }; }, orderBy() { return this; } } }
        });
  ...
  const result = await page.evaluate(async () => {
    ...
    try {
      out.windowFirebaseConfig = window.__FIREBASE_CONFIG__ || (window.firebase && window.firebase.apps && window.firebase.apps[0] && window.firebase.apps[0].options) || null;
    ...
    try {
      ...
      } catch (e) { out.currentUserError = e.message; }

      // attempt to fetch projects (catch permission errors)
      try {
        if (services && services.db) {
          const snapshot = await services.db.collection('projects').get().catch(e=>{ throw e; });
          out.projects = snapshot.docs ? snapshot.docs.map(d => ({ id: d.id, data: d.data ? d.data() : null })) : [];
        } else if (window.firebase && window.firebase.firestore) {
          const snap = await window.firebase.firestore().collection('projects').get().catch(e=>{ throw e; });
          out.projects = snap.docs.map(d=>({ id: d.id, data: d.data() }));
        } else {
          out.projects = null;
        }
      } catch (e) { out.projectsError = { message: e.message, stack: e.stack }; }
    } catch (e) {
      out.error = e.message;
    }
    return out;
